Shader "Custom/CylindricalWorldShader"
{
	...
	SubShader
	{
		...
		void surf (Input IN, inout SurfaceOutput o)
		{
			...
			float3 xyz = normalize(IN.worldNormal); //could use normals instead
			
			UV = float2(atan2(xyz.z,xyz.x)/6.283185307179586, asin(xyz.y)/3.14159265359);
			
			UV.y *= _Scale*2;
			
			float _XScale; 
			
			_XScale = ceil(abs(_Scale - UV.y)); //scale is based on proximity to poles
