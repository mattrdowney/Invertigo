Shader "Custom/SphereTiler"
{
	...
	SubShader
	{
		...
		//short of creating numerical methods that return the range [-0.5,.5] for atan2 and asin, this is optimized (to my knowledge)
		void surf (Input IN, inout SurfaceOutput o)
		{
			...
			xyz = normalize(IN.worldNormal); //worldPos is more accurate but requires the object to be centered at the origin of the map
			
			UV.x = atan2(xyz.z,xyz.x)/6.28318530718; 
			UV.y = asin(xyz.y)/3.14159265359;
			
			UV.y *= _Scale*2; //CG's optimizer combines this step with the division by PI on the previous line
			
			_XScale = ceil(_Scale - abs(UV.y));//scale is based on proximity to poles (UV.y == .5 and UV.y == -.5)
			UV.x *= _XScale;
			//UV.x += LUT[_XScale-1];
